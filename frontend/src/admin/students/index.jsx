...
import {  fetchAdminInfo, fetchAdminStudents } from "../../hooks/adminHook";
...
function AllStudents() {
  ...
  const { list, size, error, p } = fetchAdminStudents(
    10,
    page,
    data?.permission,
      ...
      <div className="py-5">
        ...
        <div className="my-2 max-h-[50vh] overflow-y-scroll shadow-md rounded-md">
          <table className="table">
            <thead>
              <tr className="bg-slate-900 table-pin-rows">
                {/* <td>UID</td> */}
                <td>Full Name</td>
                <td>Email</td>
                <td>Phone</td>
              ...
              {list
                ...
                .map((user, id) => {
                  return (
                    <tr
                      key={user?._id}
                      className={`${id % 2 !== 0 && "bg-slate-300"} font-thin`}
                    >
