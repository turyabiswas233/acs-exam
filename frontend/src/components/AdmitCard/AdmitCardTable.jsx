    ...
    const fetchData = async () => {
      if (user)
        try {
          ...
          setLoading(true);
          const response = await fetch(`${url}api/admitcards`);
          if (!response.ok) {
            ...
            <div className="w-full lg:w-2/3 px-4 mb-4">
              <div className="border-b-2 border-gray-300 pb-2 mb-6">
                <div className="flex text-lg font-semibold text-gray-700">
                  ...
                  <div className="w-1/3">Admit Card Link</div>
                </div>
              </div>
              {data?.map((item, index) => (
                <div
