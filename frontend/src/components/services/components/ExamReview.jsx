...
function ExamReview( {id} ){
    ...
    const questions = dataLoaded ? data.examInfo?.questionsList.map((q, ind)=>{
        
        const optionEL = q.options.map( (op, ind) =>{
            console.log(op.isCorrect);
            const bgStyle = op.isCorrect ? "bg-slate-300 rounded-lg border-2 border-blue-500 " : " ";
            return (
            <div key={op.id} className={ "flex p-1 "+ bgStyle }>
                <span className="text-gray-500 font-bold">{ options_identifier[ind] }. &nbsp;</span>
                <p className="">
        ...
        return (
            <div key={q.id} className="mb-3 w-full bg-slate-100 rounded-lg p-3">
                <p className="flex mb-3"> {ind+1}. &nbsp; <span dangerouslySetInnerHTML={{ __html: q.question }}></span> </p>
        ...
        <div className="p-5 text-primary">
            ...
            <p className="text-md font-mono mb-0"> 
                Class: <span className="text-white"> 
                    { data?.examInfo?.examclass || "HSC" }  </span>
            </p>
            <p className="text-md font-mono mb-0"> 
                Exam :  <span className="text-white"> 
                    { data?.examInfo?.examname || "Demo" } </span>
            </p>

            <p className="text-md font-mono mb-0"> 
                Start Time :  <span className="text-white"> 
                    { data?.examInfo?.starttime || "NO time" } </span>
            </p>

            <p className="text-md font-mono mb-0"> 
                End Time :  <span className="text-white"> 
                    { data?.examInfo?.endtime || "NO time" } </span>
            </p>

            <p className="text-md font-mono mb-3"> 
                Duration :  <span className="text-white"> 
                    { data?.examInfo?.duration.hh || 0 } Hour { data?.examInfo?.duration.mm || 0 } Minutes </span>
            </p>

             { questions }

            <button className="btn btn-info px-6 mt-3"
                onClick={ ()=> { 
                    window.location = "\\"; 
                } }
            > Go to Home </button>
